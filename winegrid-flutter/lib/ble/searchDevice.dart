...
import 'package:winegrid/ble/widgets.dart';
import 'qr_scan.dart';

class SearchBlueApp extends StatefulWidget {
  SearchBlueApp({Key key, this.title}) : super(key: key);
...
class _FlutterBlueAppState extends State<SearchBlueApp> {
  ...
  _connect(BluetoothDevice d) {
    QRScanState.result = d.name;
    if(state.toString().substring(15) == 'off') {
      Functions.showBluetoothAdapterInfo(context);
    } else {
  ...
  _buildScanResultTiles() {
    ...
    List<ScanResultTile> res = List<ScanResultTile>();

    for(int i = 0; i < list.length; i++) {
      if(list[i].result.advertisementData.localName != "") {
        res.add(list[i]);
      }
    }
...
*/
    ...
    return new Scaffold(
      ...
      appBar: Functions.wineBar("Available Devices"),
      body: new Stack(
        children: <Widget>[
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        backgroundColor: Functions.winegridColor(),
